...
<article class="tweet-card">
    ...
    <div class="share-buttons" style="margin-top: 15px; display: flex; gap: 10px; border-top: 1px solid var(--border); padding-top: 10px;">
        <a href="https://twitter.com/intent/tweet?text=<?= urlencode($text) ?>&url=<?= urlencode($baseUrl . '/user/' . $screenName) ?>" target="_blank" class="tag-link" style="font-size: 0.75rem; background: #000; color: #fff;"><i class="fa-brands fa-x-twitter"></i> Paylaş</a>
        <a href="https://www.facebook.com/sharer/sharer.php?u=<?= urlencode($baseUrl . '/user/' . $screenName) ?>" target="_blank" class="tag-link" style="font-size: 0.75rem; background: #1877f2; color: #fff;"><i class="fa-brands fa-facebook"></i> Facebook</a>
        <a href="https://wa.me/?text=<?= urlencode($text . ' ' . $baseUrl . '/user/' . $screenName) ?>" target="_blank" class="tag-link" style="font-size: 0.75rem; background: #25d366; color: #fff;"><i class="fa-brands fa-whatsapp"></i> WhatsApp</a>
        <button onclick="navigator.clipboard.writeText('<?= $baseUrl . '/user/' . $screenName ?>').then(() => alert('Link kopyalandı!'))" class="tag-link" style="font-size: 0.75rem; border: none; cursor: pointer;"><i class="fa-solid fa-link"></i> Kopyala</button>
    </div>
</article>
